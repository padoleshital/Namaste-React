...
A : No,We can also write using normal javaScript. 
...
---
### Q : What is Props?
A : Passing Argument to function is call as props

---
### Q : `{TitleComponent}` vs `{<TitleComponent/>}` vs `{<TitleComponent></TitleComponent>}` in JSX.
A : 
- `{TitleComponent}` This is React element or variable. `TitleComponent` as a  Javascript expression
...
A : 
- Both  `<React.Fragment></React.Fragment>` and `<></>` are use in React grouping without adding extra nodes in DOM. 
- This is useful when you need to return multiple elements from a component without wrapping them in an unnecessary <div>.
- `<React.Fragment></React.Fragment>` No need to add extra <div> Use in List. 
- `<></>` Shorthand of  `<React.Fragment></React.Fragment>`.
---
